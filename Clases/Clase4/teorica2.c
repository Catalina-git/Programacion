...
int main(void) {
    ...
    printf("Memory (pile) \t\t Computer Addres \t Variable Name\n");
    ...
    /* Imprimen lo mismo */
    printf("\nptr vs &var\n");
    printf(" ptr = \t %p\n", ptr); /* Imprime lo que esta adentro de ptr */

    /* Lo que esta 'adentro' de ptr es la direccion de memoria de var
       - (*) me devuelve lo que esta adentro de la direccion de memoria a la que apunta su operando.
       - *ptr --> me devuelve lo que esta adentro de &var. 
       - *(&var) --> me devuelve lo que esta adentro de la direccion de memoria de var. 
    */
    ...
    *ptr = 20;
    ...
    /* var --> se donde esta var, entonces puedo modificarla 
       ptr --> se donde esta var, y puedo modificarla. Pero tambien puedo hacer 
               que ptr apunte a otra variable (var2) y modificarla. 
    */
    
    /* Conociendo el valor de ptr puedo: 
       --> por un lado saber donde esta guardada la info de var (ptr)
       --> acceder y modificar la info que esta guardada en var
    */

    #endif /* POINTER_OPERATORS */
    ...
    p_arr = arr; /* arr es un puntero constante que apunta siempre a &(arr[0]) --> a la posicion cero del array */
    print("double *p_arr = arr =\t\t %p\n", p_arr);
    p_arr = &arr[0];
    print("double *p_arr = &arr[0] =\t\t %p\n", p_arr);

    printf("\n--------------------------------------------\n");

    /* Recorro el vector */
    for (int i = 0; i < LEN; i++) {
        printf("arr[%d] = \t %lf\n", i, arr[i]); /* Imprime el valor del arr en la posicion i --> arr[i] */
        /* 1) 1E8     ->  2)  3.8E6   -> 3) 5*/
